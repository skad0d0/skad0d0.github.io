...
Here we give a simple numercial example of how the mon ladder works. Supppose the scalar is 9, we want to compute $9*P$. We decode the scalar into binary code 1001, we set $P_0=0, P_1=P$. Start from the most significant bit.
- Iteration 1: A[3]=1, so we swap $P_0, P_1$, now $P_0=P, P_1=0$, we overide the point we will not use i.e. $P_1=P_1+P_0=P$, then we do the doubling $P_0=2*P_0=2P$. We swap them back, so after first iteration, we get $P_0=P, P_1=2P$.
- Iteration 2: A[2]=0, $P_1=P_1+P_0=3P$, $P_0=2*P_0=2P$.
- Iteration 3: A[1]=0, $P_1=P_1+P_0=5P$, $P_0=2*P_0=4P$.
- Iteration 4: A[0]=1, so we swap $P_0, P_1$. $P_1=P_1+P_0=9P$, $P_0=2*P_0=10P$. After swap back, $P_0=9P, P_1=10P$.
